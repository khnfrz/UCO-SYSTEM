...
import { DesktopDepartmentsWrapper } from "./layout/DesktopViewing";
import { MobileDepartmentsWrapper } from "./layout/MobileViewing";
...
export default function DepartmentsContent() {
  ...
  const DepartmentsWrapper = isMobile ? MobileDepartmentsWrapper : DesktopDepartmentsWrapper;
  ...
  return (
    <DepartmentsWrapper>
      <div className="flex items-center justify-between border-b border-slate-300 pb-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">University Departments</h1>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Submission metrics per office</p>
        </div>
        <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-300">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Registered Offices</span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mt-12">
        {departmentsData.map((cat, idx) => (
          <div key={idx} className="bg-slate-50 rounded-[2.5rem] shadow-xl shadow-slate-300/50 border border-slate-100 overflow-hidden flex flex-col">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-3 flex-grow">
              {cat.offices.map((office, oIdx) => {
                ...
                return (
                  <a 
                    key={oIdx} 
                    href={`/office?office=${encodeURIComponent(office)}`}
                  ...
                  >
                    <div className="flex flex-col">
                      <p className="text-[11px] font-black text-slate-700 leading-tight group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{office}</p>
                      <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-widest">Active Requests</p>
                    </div>
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black transition-all ${count > 0 ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-slate-50 text-slate-300 group-hover:bg-slate-100'}`}>
                      {count}
                    </div>
                  </a>
                );
