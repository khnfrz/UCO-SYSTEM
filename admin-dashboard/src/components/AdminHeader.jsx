        ...
        {officeMatches.length > 0 && (
          <>
            ...
            {officeMatches.map(match => (
              <div key={match} className="px-4 py-3 md:py-2 text-sm text-slate-700 hover:bg-indigo-50 cursor-pointer active:bg-indigo-100" onClick={() => { showLoading(); window.location.href = `/office?office=${encodeURIComponent(match)}`; }}>
                {match}
              </div>
            ))}
          </>
        ...
        {submissionMatches.length > 0 && (
          <>
            ...
            {submissionMatches.map(s => (
              <div key={s.id} className="px-4 py-3 md:py-2 text-sm text-slate-700 hover:bg-indigo-50 cursor-pointer flex flex-col active:bg-indigo-100" onClick={() => { showLoading(); window.location.href = `/submission?id=${s.id}`; }}>
